...
afc_map <- ggplot() +
  ...
  geom_point(data = maxdaten, aes(x = lon, y = lat, fill=daysold), shape=22, 
  ...
  geom_text(data = maxdaten, aes(x = lon, y = lat, label = round(afc,0)), 
  ...
  labs(title = paste("Average Farm Cover (AFC) ", strftime(today, format = "%a %d.%m.%Y"), 
                     ", ", Kalenderwoche, " ", Jahr, sep=""), 
       subtitle = "Durchschnittliche nutzbarer Grasvorrat in kg TS/ha, Quelle: AGFF-Grasmessnetzwerk")+
  theme_void()


afc_map
mapfile <- paste("outputs/AFC_", Jahr, "KW",week, ".svg", sep="")
